"""Pure-Python helpers for reading HoGLTF material node IR.

This file intentionally has no Blender or HoGLTF runtime dependency. It can be
    ...
    for tree_path, node in iter_nodes(ir, include_groups=include_groups):
        ...
        if not record["has_serialized_detail"]:
            record["needs_reexport"] = (
                "This IR was likely exported before color_ramp/curve_mapping serialization. Re-export from Blender with the updated HoGLTF exporter."
            )
        records.append(record)
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Inspect HoGLTF material node IR JSON.")
