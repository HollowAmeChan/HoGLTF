...
description: Export, inspect, live-query, compare, or evolve HoGLTF Blender object/scene IR for mesh statistics, UVs, material slots, modifiers, shape keys, color attributes, hierarchy, and material migration context.
---
...
1. Export object context from Blender with `adapters/blender_ir/object_scene_ir.py`.
2. Use JSON as the canonical source of truth. Markdown is only a compact reading view.
3. Use `adapters/blender_ir/object_scene_ir_ai.py` outside Blender for summaries and migration-readiness checks.
...
5. When a `.blend` is available and a full scene bundle is too large, use `adapters/blender_ir/blender_live_inspector.py` through the matching Blender/Goo runtime for quick object/material/attribute checks.
6. For Blender-side automation, import `from blender_ir import object_scene_ir` after adding the HoGLTF adapters folder to `sys.path`; avoid importing top-level `HoGLTF` unless the full UI add-on is required.

## Helper Entrypoints

- `python adapters/blender_ir/object_scene_ir_ai.py object_scene_ir.json --mode preview`: chat-friendly object/scene estimate.
- `python adapters/blender_ir/object_scene_ir_ai.py object_scene_ir.json --mode summary`: object counts, mesh totals, materials, UVs, attributes, modifiers.
- `python adapters/blender_ir/object_scene_ir_ai.py object_scene_ir.json --mode audit`: migration risks such as missing UVs, shape keys, and Geometry Nodes modifiers.
- `python adapters/blender_ir/ir_joint_ai.py --scene-bundle scene.scene_asset.json --mode preview`: jointly inspect the one-click scene bundle containing all scene objects, material node IRs, and Geometry Nodes IR when enabled.
- `python adapters/blender_ir/ir_joint_ai.py --object-scene scene.object_scene.json --material mat.json --mode preview`: jointly inspect separately exported object and material IR files.
- `blender --factory-startup --background asset.blend --python adapters/blender_ir/blender_live_inspector.py -- --mode scene`: inspect live scene/object context without exporting a scene bundle.
- `blender --factory-startup --background asset.blend --python adapters/blender_ir/blender_live_inspector.py -- --mode materials`: inspect live material sizes and Goo/fork signals before choosing which material to export.
