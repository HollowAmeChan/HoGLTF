...
```powershell
& 'D:\Blender\Goo-Engine 4.4\blender.exe' --factory-startup --background 'D:\Asset\scene.blend' --python 'C:\Users\hhh12\AppData\Roaming\Blender Foundation\Blender\4.5\scripts\addons\HoGLTF\adapters\blender_ir\blender_live_inspector.py' -- --mode materials --pretty
```
...
- Use absolute paths when running from Codex or another project. Do not assume the current directory is HoGLTF.
- For custom Blender scripts, add the HoGLTF adapters folder to `sys.path` and import `from blender_ir import ...`; do not import the top-level `HoGLTF` package unless full UI add-on registration is needed.
...
```powershell
& 'D:\Blender\Goo-Engine 4.4\blender.exe' --factory-startup --background 'scene.blend' --python 'adapters\blender_ir\blender_live_inspector.py' -- --mode app --pretty
```
...
```powershell
& 'D:\Blender\Goo-Engine 4.4\blender.exe' --factory-startup --background 'scene.blend' --python 'adapters\blender_ir\blender_live_inspector.py' -- --mode materials --no-groups --pretty
```
...
```powershell
& 'D:\Blender\Goo-Engine 4.4\blender.exe' --factory-startup --background 'scene.blend' --python 'adapters\blender_ir\blender_live_inspector.py' -- --mode node --material 'M_actor_bounda_cloth_02.001' --node 'Screenspace' --limit 20 --pretty
```
...
- Script: `adapters/blender_ir/blender_live_inspector.py`
- Material IR helper: `adapters/blender_ir/material_ir_ai.py`
- Scene/Object helper: `adapters/blender_ir/object_scene_ir_ai.py`
