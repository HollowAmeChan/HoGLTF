...
```powershell
& 'D:\Blender\blender-4.5.8-windows-x64\blender.exe' --factory-startup --background 'scene.blend' --python 'C:\Users\hhh12\AppData\Roaming\Blender Foundation\Blender\4.5\scripts\addons\HoGLTF\adapters\blender_ir\compositor_node_ir.py' -- --output 'C:\Temp\scene.compositor_node.json' --format JSON
```

For custom Blender automation, import only `HoGLTF.adapters.blender_ir`, not the whole HoGLTF add-on:
...
Use the Node Editor panel `HoGLTF > Compositor Node IR` when working interactively in a Compositor node tree.

## Helper

- `python adapters/blender_ir/compositor_node_ir_ai.py scene.compositor_node.json --mode preview`: compact human-readable overview.
- `python adapters/blender_ir/compositor_node_ir_ai.py scene.compositor_node.json --mode summary`: node/link/type counts.
- `python adapters/blender_ir/compositor_node_ir_ai.py scene.compositor_node.json --mode io`: input and output nodes.
- `python adapters/blender_ir/compositor_node_ir_ai.py scene.compositor_node.json --mode effects`: color/filter/keying/transform groups.
- `python adapters/blender_ir/compositor_node_ir_ai.py scene.compositor_node.json --mode audit`: migration/bake risks.
...
- Exporter: `adapters/blender_ir/compositor_node_ir.py`
- AI helper: `adapters/blender_ir/compositor_node_ir_ai.py`
