...
description: Export, inspect, audit, or evolve HoGLTF Geometry Nodes IR for Blender/Goo Geometry Nodes modifiers, node groups, fields, attributes, instances, bake nodes, and zone nodes such as Simulation, Repeat, For Each Geometry Element, and Closure.
---
...
In Blender, use the HoGLTF View3D panel `Geometry Nodes IR` or run:

```powershell
& 'D:\Blender\blender-4.5.8-windows-x64\blender.exe' --factory-startup --background 'asset.blend' --python 'C:\Users\hhh12\AppData\Roaming\Blender Foundation\Blender\4.5\scripts\addons\HoGLTF\adapters\blender_ir\geometry_node_ir.py' -- --scope SCENE --output 'C:\Temp\asset.geometry_node.json' --format JSON
```

For custom Blender automation, import the small package directly instead of importing all of HoGLTF:
...
- In background mode, avoid `import HoGLTF` when only IR tools are needed. `from blender_ir import geometry_node_ir` is the intended package import, and `--python adapters/blender_ir/geometry_node_ir.py -- ...` is the simplest CLI path.

## Helper Entrypoints

- `python adapters/blender_ir/geometry_node_ir_ai.py asset.geometry_node.json --mode preview`: human-readable overview.
- `python adapters/blender_ir/geometry_node_ir_ai.py asset.geometry_node.json --mode summary`: counts, modifiers, node types, zone counts.
- `python adapters/blender_ir/geometry_node_ir_ai.py asset.geometry_node.json --mode zones`: list Simulation/Repeat/For Each/Closure zone pairs and items.
- `python adapters/blender_ir/geometry_node_ir_ai.py asset.geometry_node.json --mode effects`: list nodes that affect generated geometry, attributes, instances, sampling, or materials.
- `python adapters/blender_ir/geometry_node_ir_ai.py asset.geometry_node.json --mode audit`: migration risks and required bake/custom-code review.
...
- Exporter: `adapters/blender_ir/geometry_node_ir.py`
- AI helper: `adapters/blender_ir/geometry_node_ir_ai.py`
- Schema reference: `adapters/blender_ir/skills/geometry-node-ir/references/ir-schema.md`
