...
1. 选中目标物体或打开目标场景。
2. 在 Shader Editor 或 View3D 侧栏找到 HoGLTF 的 IR 面板。
...
```powershell
python adapters/blender_ir/material_ir_ai.py material_ir.json --mode preview
python adapters/blender_ir/material_ir_ai.py material_ir.json --mode audit
python adapters/blender_ir/material_ir_ai.py material_ir.json --mode images
python adapters/blender_ir/material_ir_ai.py material_ir.json --mode groups
python adapters/blender_ir/material_ir_ai.py material_ir.json --mode annotations
python adapters/blender_ir/material_ir_ai.py material_ir.json --mode colors
python adapters/blender_ir/material_ir_ai.py material_ir.json --mode inputs
python adapters/blender_ir/material_ir_ai.py material_ir.json --mode goo
python adapters/blender_ir/material_ir_ai.py material_ir.json --mode source --source-profile goo
```

Object / Scene IR：

```powershell
python adapters/blender_ir/object_scene_ir_ai.py object_scene.json --mode preview
python adapters/blender_ir/object_scene_ir_ai.py object_scene.json --mode audit
python adapters/blender_ir/ir_joint_ai.py --scene-bundle scene.scene_asset.json --mode preview
```

Geometry Nodes IR：

```powershell
& 'D:\Blender\blender-4.5.8-windows-x64\blender.exe' --factory-startup --background 'scene.blend' --python 'adapters\blender_ir\geometry_node_ir.py' -- --scope SCENE --output 'scene.geometry_node.json' --format JSON
python adapters/blender_ir/geometry_node_ir_ai.py asset.geometry_node.json --mode preview
python adapters/blender_ir/geometry_node_ir_ai.py asset.geometry_node.json --mode summary
python adapters/blender_ir/geometry_node_ir_ai.py asset.geometry_node.json --mode zones
python adapters/blender_ir/geometry_node_ir_ai.py asset.geometry_node.json --mode effects
python adapters/blender_ir/geometry_node_ir_ai.py asset.geometry_node.json --mode audit
```
...
AI 写 Blender 后台脚本时，不要为了用 IR 工具去 `import HoGLTF`。顶层 HoGLTF 可能会加载 UI/GPU 绘制相关模块，在 `--background` 下容易被无关模块影响。

正确做法是把 HoGLTF 插件目录加入 `sys.path`，然后只导入 `HoGLTF.adapters.blender_ir` 这个小包：

```python
import sys
sys.path.insert(0, r"C:\Users\hhh12\AppData\Roaming\Blender Foundation\Blender\4.5\scripts\addons\HoGLTF\adapters")

from blender_ir import material_node_ir
from blender_ir import object_scene_ir
from blender_ir import geometry_node_ir
from blender_ir import compositor_node_ir
```
...
外部 JSON 分析脚本例如 `material_ir_ai.py`、`geometry_node_ir_ai.py`、`compositor_node_ir_ai.py` 仍然是纯 Python 标准库脚本，不需要 Blender，也不需要 import `HoGLTF.adapters.blender_ir`。
...
```powershell
& 'D:\Blender\Goo-Engine 4.4\blender.exe' --factory-startup --background 'D:\Asset\scene.blend' --python 'C:\Users\hhh12\AppData\Roaming\Blender Foundation\Blender\4.5\scripts\addons\HoGLTF\adapters\blender_ir\blender_live_inspector.py' -- --mode app --pretty
```
...
```powershell
& 'D:\Blender\Goo-Engine 4.4\blender.exe' --factory-startup --background 'scene.blend' --python 'adapters\blender_ir\blender_live_inspector.py' -- --mode scene --pretty
```

列出材质：

```powershell
& 'D:\Blender\Goo-Engine 4.4\blender.exe' --factory-startup --background 'scene.blend' --python 'adapters\blender_ir\blender_live_inspector.py' -- --mode materials --pretty
```

查看单个材质：

```powershell
& 'D:\Blender\Goo-Engine 4.4\blender.exe' --factory-startup --background 'scene.blend' --python 'adapters\blender_ir\blender_live_inspector.py' -- --mode material --material "M_actor_bounda_cloth_02.001" --pretty
```

搜索节点：

```powershell
& 'D:\Blender\Goo-Engine 4.4\blender.exe' --factory-startup --background 'scene.blend' --python 'adapters\blender_ir\blender_live_inspector.py' -- --mode node --material "M_actor_bounda_cloth_02.001" --node "Screenspace" --pretty
```

比较 live 材质和已导出的 Material IR：

```powershell
& 'D:\Blender\Goo-Engine 4.4\blender.exe' --factory-startup --background 'scene.blend' --python 'adapters\blender_ir\blender_live_inspector.py' -- --mode compare-material-ir --material "M_actor_bounda_cloth_02.001" --ir "C:\Users\hhh12\Desktop\M_actor_bounda_cloth_02_001.json" --pretty
```
...
```text
请使用 HoGLTF 的 IR 工具分析这个 Blender/Goo 材质迁移到 Unity 的可能性。
...
- Material exporter: `adapters/blender_ir/material_node_ir.py`
- Object/scene exporter: `adapters/blender_ir/object_scene_ir.py`
- Geometry Nodes exporter: `adapters/blender_ir/geometry_node_ir.py`
- Live inspector: `adapters/blender_ir/blender_live_inspector.py`
- Material helper: `adapters/blender_ir/material_ir_ai.py`
- Object helper: `adapters/blender_ir/object_scene_ir_ai.py`
- Geometry Nodes helper: `adapters/blender_ir/geometry_node_ir_ai.py`
- Joint helper: `adapters/blender_ir/ir_joint_ai.py`
- Material skill: `adapters/blender_ir/skills/material-node-ir/SKILL.md`
- Object skill: `adapters/blender_ir/skills/object-scene-ir/SKILL.md`
- Live inspector skill: `adapters/blender_ir/skills/blender-live-inspector/SKILL.md`
